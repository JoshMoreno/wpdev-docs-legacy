...
class MethodParser extends \ReflectionMethod
{
	...
	{
		$required = collect($this->getParameters())->filter(function($param){
			return !isset($param['default']);
		})->map(function($param) {
			$type = $param['type'] ?? '';
			$name = !empty($param['name']) ? '<code>$'.$param['name'].'</code>' : '';
			return "{$type} {$name}";
		})->implode(', ');


		$optional = collect($this->getParameters())->filter(function($param){
			return isset($param['default']);
		})->map(function($param) {
			$type = $param['type'] ?? '';
			$name = !empty($param['name']) ? '<code>$'.$param['name'].'</code>' : '';
	...
	/**
	 * Checks to see if there is a docblock return tag
	 *
	 * @return bool
	 */
